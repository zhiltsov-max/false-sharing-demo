...
известная под названием false sharing (ложное разделение данных).
...
Далее, рассмотрим архитектураы вычислительных систем относительно
...
элементами (ядрами, процессорами, узлами, ...) данные для обработки
...
обработчиков выполняют вычисления над одними данными и хотябы один из них
...
Выполнялось по несколько запусков, выбиралось наилучшеее время.
...
выравнивание объектов и раздмещение их в памяти с определенным шагом. В
демонстрационном примере использован ээтот подход. Однако, следует иметь
в виду, что такие действия хить и могут защитить от исследуемой проблемы,
...
специализированных средств профилирования кода. Применение экспертный метода
...
кода на языке С++, в котором воспроизводится проблема. Проведены эксперимены,
